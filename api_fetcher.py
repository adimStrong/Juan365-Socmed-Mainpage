...
def fetch_all_posts():
    """Fetch all posts - basic info first."""
...
def process_posts(posts, engagement_data=None, reactions_data=None):
    """Process posts into a clean format for the dashboard."""
    processed = []
...
def main():
    ...
    # Fetch engagement (reactions/comments count) for last 100 posts only
    # This is fast and completes in ~2 minutes
    engagement_data = {}
    if posts:
        recent_post_ids = [p['id'] for p in posts[:100]]
        engagement_data = fetch_post_engagement(recent_post_ids)

    # Fetch reaction breakdown for last 100 posts
    reactions_data = {}
    if posts:
        recent_post_ids = [p['id'] for p in posts[:100]]
    ...
    # Save to files
    save_data(page_info, posts, videos, engagement_data, reactions_data)
